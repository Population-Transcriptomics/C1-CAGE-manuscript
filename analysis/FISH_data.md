...
![](FISH_data_files/figure-html/unnamed-chunk-1-1.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-2-1.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-2-2.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-2-3.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-3-1.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-3-2.png)<!-- -->
...
```
## Warning in melt.data.table(df2, id.vars = c("t", "gene_id", "chr")): 'measure.vars' [enhancer, gene, overlap, pc_gene,
## pc_enh] are not all of the same type. By order of hierarchy, the molten data value column will be of type 'double'. All
## measure variables not of type 'double' will be coerced to. Check DETAILS in ?melt.data.table for more on coercion.
```
...
![](FISH_data_files/figure-html/unnamed-chunk-4-1.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-4-2.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-4-3.png)<!-- -->
...
```
## Warning in melt.data.table(df2, id.vars = c("t", "chr")): 'measure.vars' [sense_n, sense_t, anti_n, anti_t, ...] are
## not all of the same type. By order of hierarchy, the molten data value column will be of type 'double'. All measure
## variables not of type 'double' will be coerced to. Check DETAILS in ?melt.data.table for more on coercion.
```
...
![](FISH_data_files/figure-html/unnamed-chunk-5-1.png)<!-- -->
...
![](FISH_data_files/figure-html/unnamed-chunk-5-2.png)<!-- -->
...
fish_genes <- c("ENSG00000067082.14_1", "ENSG00000124225.15_2", "ENSG00000005882.11_2", 
    "ADDG10003929991.E", "ADDG17048105016.E", "ADDG20056293544.E")
...
enh_m[, .(n = .SD[reads > 0, .N]/.N, cells = .SD[reads > 0, .N], tot = .N), by = .(gene, 
    t, chr, type)][order(gene, t)]
```

```
##                     gene   t   chr     type          n cells tot
##  1: ENSG00000067082.14_1 t00 chr10     gene 0.63888889    23  36
##  2: ENSG00000067082.14_1 t06 chr10     gene 0.86842105    33  38
##  3: ENSG00000067082.14_1 t24 chr10     gene 0.95238095    60  63
##  4: ENSG00000124225.15_2 t00 chr20     gene 0.11111111     4  36
##  5: ENSG00000124225.15_2 t06 chr20     gene 0.39473684    15  38
##  6: ENSG00000124225.15_2 t24 chr20     gene 0.53968254    34  63
##  7: ENSG00000005882.11_2 t00 chr17     gene 0.27777778    10  36
##  8: ENSG00000005882.11_2 t06 chr17     gene 0.31578947    12  38
##  9: ENSG00000005882.11_2 t24 chr17     gene 0.26984127    17  63
## 10:    ADDG10003929991.E t00 chr10 enhancer 0.00000000     0  36
## 11:    ADDG10003929991.E t06 chr10 enhancer 0.05263158     2  38
## 12:    ADDG10003929991.E t24 chr10 enhancer 0.06349206     4  63
## 13:    ADDG17048105016.E t00 chr17 enhancer 0.08333333     3  36
## 14:    ADDG17048105016.E t06 chr17 enhancer 0.02631579     1  38
## 15:    ADDG17048105016.E t24 chr17 enhancer 0.03174603     2  63
## 16:    ADDG20056293544.E t00 chr20 enhancer 0.02777778     1  36
## 17:    ADDG20056293544.E t06 chr20 enhancer 0.10526316     4  38
## 18:    ADDG20056293544.E t24 chr20 enhancer 0.09523810     6  63
```

```r
enh_m[, .(dcast.data.table(.SD, cell ~ type, value.var = "reads")[, sum(enhancer > 
    0 & gene > 0)/sum(enhancer > 0)]), by = .(chr, t)]
```

```
##      chr   t        V1
## 1: chr10 t24 1.0000000
## 2: chr20 t24 0.3333333
## 3: chr17 t24 0.0000000
## 4: chr10 t00       NaN
## 5: chr20 t00 0.0000000
## 6: chr17 t00 0.6666667
## 7: chr10 t06 0.5000000
## 8: chr20 t06 0.5000000
## 9: chr17 t06 1.0000000
```

```r
(ggplot(enh_m[chr == "chr20", .(dcast.data.table(.SD, cell ~ type, value.var = "reads")[, 
    sum(enhancer > 0 & gene > 0)/sum(enhancer > 0)]), by = .(chr = paste0(chr, " eRNA"), 
    t = gsub("t", "", t))], aes(t, V1 * 100)) + geom_bar(stat = "identity") + facet_wrap(~chr) + 
    xlab("Timepoint (hours)") + ylab("% eRNA expressing cells \nalso expressing PMEPA1"))
```

![](FISH_data_files/figure-html/fish_figs-1.png)<!-- -->

```r
ggsave("../manuscript/Fig5/chr20_overlap_cage.pdf", width = 3, height = 3, units = "in")


(ggplot(enh_m[, .(n = .SD[reads > 0, .N]/.N), by = .(gene, t = gsub("t", "", t), 
    chr, type)], aes(t, n * 100)) + geom_bar(stat = "identity") + facet_grid(type ~ 
    chr, scales = "free_y") + ylab("% cells with CAGE signal") + xlab("Timepoint"))
```

![](FISH_data_files/figure-html/fish_figs-2.png)<!-- -->

```r
ggsave("../manuscript/Fig5/all_erna_cage.pdf", width = 6, height = 4, units = "in")

(ggplot(enh_m[chr == "chr20" & type == "enhancer", .(n = .SD[reads > 0, .N]/.N), 
    by = .(gene, t = gsub("t", "", t), chr = paste0(chr, " eRNA"), type)], aes(t, 
    n * 100)) + geom_bar(stat = "identity") + facet_grid(. ~ chr, scales = "free_y") + 
    ylab("% cells with CAGE signal") + xlab("Timepoint"))
```

![](FISH_data_files/figure-html/fish_figs-3.png)<!-- -->
...
```
##                 gene   t          n cells tot
## 1: ADDG10003929991.E t00 0.00000000     0  36
## 2: ADDG10003929991.E t06 0.05263158     2  38
## 3: ADDG10003929991.E t24 0.06349206     4  63
## 4: ADDG17048105016.E t00 0.08333333     3  36
## 5: ADDG17048105016.E t06 0.02631579     1  38
## 6: ADDG17048105016.E t24 0.03174603     2  63
## 7: ADDG20056293544.E t00 0.02777778     1  36
## 8: ADDG20056293544.E t06 0.10526316     4  38
## 9: ADDG20056293544.E t24 0.09523810     6  63
```

```r
(ggplot(enh_m[, .(n = .SD[reads > 0, .N]/.N), by = .(gene, t)], aes(t, n * 100)) + 
    geom_bar(stat = "identity") + facet_wrap(~gene) + ylab("% cells with CAGE signal") + 
    xlab("Timepoint"))
```

![](FISH_data_files/figure-html/fish_figs-4.png)<!-- -->
...
```
##                    gene   t         n cells tot
## 1: ENSG00000067082.14_1 t00 0.6388889    23  36
## 2: ENSG00000067082.14_1 t06 0.8684211    33  38
## 3: ENSG00000067082.14_1 t24 0.9523810    60  63
## 4: ENSG00000124225.15_2 t00 0.1111111     4  36
## 5: ENSG00000124225.15_2 t06 0.3947368    15  38
## 6: ENSG00000124225.15_2 t24 0.5396825    34  63
## 7: ENSG00000005882.11_2 t00 0.2777778    10  36
## 8: ENSG00000005882.11_2 t06 0.3157895    12  38
## 9: ENSG00000005882.11_2 t24 0.2698413    17  63
```
...
![](FISH_data_files/figure-html/fish_figs-5.png)<!-- -->
...
```
## Using 'reads' as value column. Use 'value.var' to override
```
...
enh_m <- enh_m[, .(gene = sum(ENSG00000067082.14_1)/.N, enh = sum(ADDG10003929991.E)/.N, 
    both = sum(both)/.N), by = .(t)]
enh_m <- melt(enh_m, id.vars = "t")
(ggplot(enh_m, aes(t, value * 100, color = variable)) + geom_point() + geom_line(aes(group = variable)) + 
    ylab("% cells with CAGE signal") + xlab("Timepoint") + ggtitle("KLF6 + eRNA"))
```

![](FISH_data_files/figure-html/fish_figs-6.png)<!-- -->

```r
# ggsave(filename = '~/Downloads/overlap_1.png', width = 4, height = 3, units =
# 'in')
...
enh_m <- enh_m[, .(gene = sum(ENSG00000124225.15_2)/.N, enh = sum(ADDG20056293544.E)/.N, 
    both = sum(both)/.N), by = .(t)]
enh_m <- melt(enh_m, id.vars = "t")
(ggplot(enh_m, aes(t, value * 100, color = variable)) + geom_point() + geom_line(aes(group = variable)) + 
    ylab("% cells with CAGE signal") + xlab("Timepoint") + ggtitle("PMEPA1 + eRNA"))
```

![](FISH_data_files/figure-html/fish_figs-7.png)<!-- -->

```r
# ggsave(filename = '~/Downloads/overlap_2.png', width = 4, height = 3, units =
# 'in')
```
