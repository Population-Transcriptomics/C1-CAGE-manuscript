...
```{r sc3_consensus_matrix_and_silhouette}
...
```{r sc3_clusters_breakdown_by_time}
...
```{r sc3_clusters_k4_diff_map, fig.height=5}
...
--|--------------------
4 | `r sum(colData(sce)$sc3_4_clusters==3)`
5 | `r sum(colData(sce)$sc3_5_clusters==4)`
6 | `r sum(colData(sce)$sc3_6_clusters==2)`

So the number of G2M cells stays between 16 to 17 cells even if you increase k.

```{r G2M and S distribution}
...
```{r find_sc3_markers_and_de_promoters}
...
```{r plot_tscan_clusters, fig.width=10, fig.height=9}
...
```{r plot_pseudotime_4_markers, fig.width=10, fig.height=7}
...
```{r plot_time_vs_tscan, fig.width=10}
...
# manuscript figure 2
pdf(file.path(dirs$manuscript, "Fig2/time_vs_tscan_PCA.pdf"), width=8, height=10)
scater::multiplot(p1, p2, p3, p4, p5, cols=2)
dev.off()
...
```{r ALDH3A1_vs_SERPINE1, fig.width=10, fig.height=10}
...
rm(tab, p1, p2, p3, p4, p5, exp.mclust)
```
